...
public class test : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(shouldWait == 0)
        {
            if (currentWaitTime > 0)
            {
                ...
                if(currentWaitTime <0)
                     Debug.Log("current wait time is:"+ 0);
                else
                    Debug.Log("current wait time is:" + currentWaitTime);
    ...
    void OnTriggerEnter()
    {
        shouldWait = random.Next(0, 4);
        if (shouldWait == 0)
           currentWaitTime= random.Next(2,6);
        ...
        while (oldRandomIndex==randomIndex)
            randomIndex = random.Next(0, waypoints.Length);

        WPindexPointer = randomIndex;
    ...
    IEnumerator Slow()
    {
        ...
        if (currentSpeed <= minSpeed)
        {
            ...
            //randomWaitTime = random.Next(2000, 5000);
            //shouldWait = random.Next(0, 4);
